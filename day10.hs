...
process :: [Int] -> Int -> ListState
process lens num = foldl step start lens
  where
...
main = do
  line <- getLine
  let
    vals = map (readInt) ((split ',') line)
    n = length vals
    state = ListState 0 (head vals) 0 (tail vals) (take 5 [0,1..])
  print $ vals
  print $ product . (take 2) $ lstOf (process vals 256)
